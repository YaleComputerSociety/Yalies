...
export function toDbRow(student: EnrichedStudent): DbRow {
	...
	return {
		...
		school: cleanStr(student.school) || "Yale College",
		school_code: cleanStr(student.school_code) || "YC",
		year,
...
export async function loadToDatabase(
	...
	try {
		...
		console.log(`Existing Yale College rows: ${existingCount}`);
		...
		try {
			// Step 1: Delete existing Yale College rows
			const [, deleteResult] = await sequelize.query(
				"DELETE FROM person WHERE school = 'Yale College' OR school_code = 'YC'",
				{ transaction },
			);
			console.log(`Deleted existing Yale College rows`);
			...
			for (const student of students) {
				...
				if (!row.id) {
					const nameKey = `YC_${row.first_name}_${row.last_name}_${row.year || ""}_${row.college || ""}`;
			...
			const [ycResult] = await sequelize.query<{ count: string }>(
				"SELECT COUNT(*) as count FROM person WHERE school = 'Yale College'",
				{ type: QueryTypes.SELECT },
			);
			const [netidResult] = await sequelize.query<{ count: string }>(
				"SELECT COUNT(*) as count FROM person WHERE school = 'Yale College' AND netid IS NOT NULL",
				{ type: QueryTypes.SELECT },
			);

			console.log(`\nDatabase summary:`);
			console.log(`  Total rows: ${totalResult.count}`);
			console.log(`  Yale College rows: ${ycResult.count}`);
			console.log(`  Yale College with netid: ${netidResult.count}`);
			console.log(`  Non-Yale College rows: ${parseInt(totalResult.count) - parseInt(ycResult.count)}`);
		} catch (e) {
