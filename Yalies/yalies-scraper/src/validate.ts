...
const MIN_TOTAL_STUDENTS = 6000;
const MAX_TOTAL_STUDENTS = 8000;
const MIN_ENRICHMENT_RATE = 0.90;
const MIN_COLLEGES = 14;
const EXPECTED_YEARS = new Set([2026, 2027, 2028, 2029]); // Update each academic year
const MIN_STUDENTS_PER_YEAR = 500;
const MIN_STUDENTS_PER_COLLEGE = 200;

const VALID_COLLEGES = new Set([
...
export function validateFacebook(data: FacebookStudent[]): ValidationResult {
	...
	const knownColleges = Object.keys(colleges).filter((c) => VALID_COLLEGES.has(c));
	const unknownColleges = Object.keys(colleges).filter((c) => !VALID_COLLEGES.has(c) && c !== "MISSING");
	...
	if (knownColleges.length >= MIN_COLLEGES) {
		...
		const missing = [...VALID_COLLEGES].filter((c) => !knownColleges.includes(c));
	...
	for (const [college, count] of Object.entries(colleges)) {
		if (VALID_COLLEGES.has(college) && count < MIN_STUDENTS_PER_COLLEGE) {
	...
	for (const [y, count] of Object.entries(years)) {
		if (!EXPECTED_YEARS.has(parseInt(y))) {
...
export function validateEnriched(data: EnrichedStudent[]): ValidationResult {
	...
	// 3. Email format
	const badEmails = data.filter((s) => s.email && !s.email.match(/^[^@]+@(.*\.)?yale\.edu$/));
	...
	// 4. NetID format
	const badNetids = data.filter((s) => s.netid && !s.netid.match(/^[a-z]{2,4}\d{1,4}$/));
...
export async function validateDatabase(databaseUrl: string): Promise<ValidationResult> {
	...
	try {
		...
		const yc = await query("SELECT COUNT(*) as count FROM person WHERE school = 'Yale College'");
		result.passes.push(`Total rows: ${total}, Yale College: ${yc}`);
		...
		for (const col of ["first_name", "last_name"]) {
			const nulls = await query(`SELECT COUNT(*) as count FROM person WHERE school = 'Yale College' AND ${col} IS NULL`);
			if (nulls === 0) result.passes.push(`DB ${col}: no nulls`);
			else result.failures.push(`DB ${col}: ${nulls} null values`);
		}

		// 4. NetID coverage
		const withNetid = await query("SELECT COUNT(*) as count FROM person WHERE school = 'Yale College' AND netid IS NOT NULL");
		...
		const [colleges] = await sequelize.query(
			"SELECT college, COUNT(*) as count FROM person WHERE school = 'Yale College' GROUP BY college ORDER BY count DESC",
		);
		...
		const [yearRows] = await sequelize.query(
			"SELECT year, COUNT(*) as count FROM person WHERE school = 'Yale College' AND year IS NOT NULL GROUP BY year ORDER BY year",
		);
		for (const row of yearRows as { year: number; count: string }[]) {
			if (EXPECTED_YEARS.has(row.year)) {
