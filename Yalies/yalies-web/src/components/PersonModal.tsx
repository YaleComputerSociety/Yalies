...
export default function PersonModal({
	...
	useEffect(() => {
		...
		const fetchProfile = async () => {
			try {
				const response = await fetch(`${process.env.NEXT_PUBLIC_YALIES_API_URL}/v2/profile/${person.netid}`, {
					method: "GET",
					credentials: "include",
					headers: {
			...
			<div className={`${styles.modal} ${isVisible ? styles.visible : ""}`}>
				...
				<div className={styles.header}>
					...
					<div className={styles.header_info}>
						...
						{person.netid && <LikeButton netid={person.netid} />}
					</div>
					{person.pronouns && (
						<span className={styles.pronouns}>{person.pronouns}</span>
					)}
					{person.netid && <FriendButton netid={person.netid} />}
