...
def enhance_all(upload: bool = True):
    """Enhance all photos in GCS that don't have an enhanced version yet."""
    import time

    client = get_gcs_client()
    bucket = client.bucket(GCS_BUCKET_NAME)

    print("Listing photos in GCS...")
    blobs = list(bucket.list_blobs())
    originals = [b.name.replace(".jpg", "") for b in blobs if b.name.endswith(".jpg") and "_enhanced" not in b.name]
    already_done = {b.name.replace("_enhanced.jpg", "") for b in blobs if "_enhanced.jpg" in b.name}

    todo = [pid for pid in originals if pid not in already_done]
    print(f"Total originals: {len(originals)}")
    print(f"Already enhanced: {len(already_done)}")
    print(f"To enhance: {len(todo)}")
    ...
    for i, photo_id in enumerate(todo, 1):
        try:
            result = enhance_from_gcs(photo_id, pipeline=pipeline, upload=upload)
            if result:
                enhanced_count += 1
            else:
                failed_count += 1
        ...
        if i % 50 == 0:
            ...
            print(f"  Progress: {i}/{len(todo)} ({enhanced_count} ok, {failed_count} failed) "
                  f"[{rate:.1f}/s, ~{remaining/60:.0f}min remaining]")

    elapsed = time.time() - start_time
    print(f"\nDone. Enhanced {enhanced_count}, failed {failed_count} in {elapsed/60:.1f} minutes.")
...
def main():
    ...
    parser.add_argument("--all", action="store_true", help="Enhance all photos in GCS bucket")
