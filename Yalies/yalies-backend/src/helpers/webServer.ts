...
export default class WebServer {
	...
	initializeSubRouters = () => {
		...
		if(process.env.AUTH_MODE === "dev") {
			...
			this.#app.use("/v2/login", devLoginRouter.getRouter());
		} else {
		...
		this.#app.use("/v2/profile", userProfileRouter.getRouter());

		const profileLikeRouter = new ProfileLikeRouter();
		this.#app.use("/v2/likes", profileLikeRouter.getRouter());

		const friendshipRouter = new FriendshipRouter();
		this.#app.use("/v2/friends", friendshipRouter.getRouter());
